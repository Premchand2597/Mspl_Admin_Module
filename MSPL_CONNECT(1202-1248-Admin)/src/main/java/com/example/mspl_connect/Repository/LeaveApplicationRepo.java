...
public interface LeaveApplicationRepo extends JpaRepository<LeaveApplication, Integer>{
	 ...
	 @Modifying
	 @Query(value = "UPDATE leave_application SET approvedstatus = :approvedStatus, approved_by=:loggedUserName,way=0,notification_status=1 WHERE id = :id", nativeQuery = true)
	 void updateApprovalStatus(String id,String approvedStatus,String loggedUserName);
	 
	 @Modifying
	 @Transactional
	 @Query(value = "UPDATE leave_application SET approvedstatus = :status, approved_by=:loggedUserName,rejection_reason=:reason,way=0,notification_status=1 WHERE id = :id", nativeQuery = true)
	 void updateRejectedStatus(String id, String status, String loggedUserName, String reason);
