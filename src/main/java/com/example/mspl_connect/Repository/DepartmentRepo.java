...
public interface DepartmentRepo extends JpaRepository<DepartmentTableEntity, Integer> {
    ...
    @Query("SELECT d.deptName FROM DepartmentTableEntity d")
	...
	value="SELECT dept_name FROM  departments where dept_id=:deptId")
	String findDeptNameByDeptId(int deptId);
	
	@Query(nativeQuery = true,
			value="SELECT dept_id FROM  departments where dept_name=:deptName")
