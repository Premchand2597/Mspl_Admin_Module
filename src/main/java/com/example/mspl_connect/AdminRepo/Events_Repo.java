...
//	Event_Entity fetchEventsDataBasedOnId(@Param("id") String id);
	...
	@Query(nativeQuery = true, value="Select * from events_table where id=cast(:id as integer)")
	Event_Entity fetchEventsDataBasedOnId(@Param("id") String id);
...
//	Integer countAnnouncementFromAnnouncementNotiByEmpid(String empid);
	...
	@Query(nativeQuery = true, value="select count(flag) from announcement_notifications where flag::int = 1 and emp_id=:empid")
	Integer countAnnouncementFromAnnouncementNotiByEmpid(String empid);
