...
public class SendEmail_Controller {
	...
	private SendEmail_Service sendEmail_Service;
	 ...
	 @PostMapping("/sendLinkToEmail")
	    public ResponseEntity<String> sendLinks(@RequestBody Map<String, String> requestData, HttpSession session) {
	        try {
	            String email = requestData.get("email");
	            String link = requestData.get("link");
	            sendEmail_Service.sendEmailWithLink(email, link, session);
