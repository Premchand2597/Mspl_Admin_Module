...
public class LeaveRequestService {
	...
	@Autowired
	private LeaveApplicationWithProfileRepo leaveApplicationWithProfileRepo;
	
	@Autowired
	private PendingLeaveWithProfilePicData_Repo pendingLeaveWithProfilePicData_Repo;
	...
	public List<PendingLeaveWithProfilePicData_DTO> fetchAllPendingLeavesDetailsWithProfilePics() {
	    List<PendingLeaveWithProfilePicData_Entity> pendingLeavesWithProfilePicLists =
	            pendingLeaveWithProfilePicData_Repo.getAllPendingLeavesDetailsWithProfilePics();

	    SimpleDateFormat originalFormat = new SimpleDateFormat("yyyy-MM-dd");
	    SimpleDateFormat targetFormat = new SimpleDateFormat("dd-MM-yyyy");

	    return pendingLeavesWithProfilePicLists.stream().map(leave -> {
	        try {
	            ...
	            // Convert Entity → DTO
	            return new PendingLeaveWithProfilePicData_DTO(
	                    leave.getEmpid(),
	                    leave.getEmployee_name(),
	                    formattedFromDate,
	                    formattedToDate,
	                    leave.getProfile_pic_path()
	            );
	        } catch (Exception e) {
	            e.printStackTrace();
	            return null; // skip in case of error
	        }
		...
		return leaveList.stream().map(leave -> {
	            try {
	            	if (leave.getFrom_date() != null && !leave.getFrom_date().isEmpty()) {
	                    ...
	                    leave.setFrom_date("N/A"); // Default value for missing date
	                }
	                
	                if (leave.getTo_date() != null && !leave.getTo_date().isEmpty()) {
	                    String formattedToDate = targetFormat.format(originalFormat.parse(leave.getTo_date()));
	                    leave.setTo_date(formattedToDate);
	                } else {
	                    leave.setTo_date("N/A"); // Default value for missing date
	                }
	            } catch (Exception e) {
	                // Handle parsing and formatting exceptions here
	                e.printStackTrace();
	...
	public List<LeaveApplicationWithProfile> getNewLeaveRequestForSA(int adminDept,String empId){
		//System.out.println("pavanaaa");
		
		List<LeaveApplicationWithProfile> leaveList = applicationWithProfileRepo.getNewleaveRequestWithProfileForSA1();
	    ...
	    @Transactional
	    public void approveLeave(String requestId,String loggedUserName,String status,String loggedEmail,String leaveReason) {
	        ...
	        try {
	            ...
	            leaveApplicationRepo.updateApprovalStatus(requestId, status,loggedUserName,currentDateTime);
	            // Send email if the update is successful
	            sendApprovalEmail(email, status,requestId,"",loggedUserName,loggedEmail,leaveReason);
	    ...
	    public void rejectLeave(String requestId, String loggedUserName, String status, String reason, String loggedEmail,String leaveReason) {
			...
			try {
	            ...
	            leaveApplicationRepo.updateRejectedStatus(requestId, status,loggedUserName,reason,currentDateTime);
	            sendApprovalEmail(email, status,requestId,reason,loggedUserName,loggedEmail,leaveReason);
	        } catch (NumberFormatException e) {
	            throw new IllegalArgumentException("Invalid employee ID format", e);
	        } catch (Exception e) {
	            throw new RuntimeException("Error during leave approval", e);
	    ...
	    private void sendApprovalEmail(String recipientEmail, String status,String requestId,String reason,String loggedUserName,String loggedEmail,String leaveReason) throws MessagingException {
	             ...
	             DisplayEmployessEntity loggedEmployeeData =  employeeRepositoryWithDeptName.findByEmail(loggedEmail);
	            ...
	            if(loggedEmployeeData.getGender().equals("Male")) {
	            	loggedAdminPrefix = "Mr.";
	            }else {
	            	loggedAdminPrefix = "Ms.";
	            }
	            ...
	            if(status.equals("Approved")) {
	            	helper.setText(
	            		    ...
	            		    "<br><b>Leave Reason:</b> " + leaveReason +
	            	...
	            	helper.setText(
	            		    "Dear " + prefix + employeeData.getFullName() +
	            		    ",<br><br>We regret to inform you that your recent leave request has been rejected by .<b> "+loggedAdminPrefix+" "+loggedUserName+ "</b>" +
	            		    "<br><br><b>Leave Details:</b><br><br>" +
	            		    "<b>Leave Type:</b> " + leaveType +
	            		    "<br><b>Leave Dates:</b> " + formattedFromDate + " to " + formattedToDate +
   ...
   /* public List<LeaveApplication> getleaveRequest(int adminDept, String empid) {
	    ...
	    /*public List<LeaveApplication> getleaveRequest(int adminDept, String empid) {
	        List<LeaveApplication> leaves = leaveApplicationRepo.getleaveRequestByEmpID(empid);
	        leaves.stream().forEach(i->System.out.println("11111"+i.getFrom_date()));
	     ...
	     // Sort first using the original pattern for requested_date
	        leaves.sort((leave1, leave2) -> {
	            try {
	        ...
	        leaves.stream().forEach(i->System.out.println("222222"+i.getFrom_date()));
	        // Then format the date fields
	        for (LeaveApplication leave : leaves) {
	            try {
	                if (leave.getFrom_date() != null && !leave.getFrom_date().isEmpty()) {
	                ...
	                if (leave.getRequested_date() != null && !leave.getRequested_date().isEmpty()) {
	                    if (leave.getRequested_date().length() == 10) {
	                        leave.setRequested_date(leave.getRequested_date() + " 00:00:00");
	                    }
	        ...
	        leaves.stream().forEach(i->System.out.println("33333"+i.getFrom_date()));
	        return leaves;
	        
	    }*/
	    
	    public List<LeaveApplicationWithProfile> getleaveRequest(int adminDept, String empid) {
	    	
	        List<LeaveApplicationWithProfile> leaves = leaveApplicationWithProfileRepo.getleaveRequestByEmpID(empid);
	         
	        // Define Date Formatters (Thread-Safe)
	        DateTimeFormatter inputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
	        DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
	        DateTimeFormatter inputDateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
	        DateTimeFormatter outputDateTimeFormatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
	        
	        // Sort by requested_date in descending order
	        leaves.sort(Comparator.comparing(leave -> {
	            try {
	                return LocalDateTime.parse(leave.getRequested_date(), inputDateTimeFormatter);
	            } catch (Exception e) {
	                return LocalDateTime.MIN; // Handle parse failure
	            }
	        }, Comparator.reverseOrder()));
	        
	        
	        // Format dates safely
	        for (LeaveApplicationWithProfile leave : leaves) {
	            try {
	                if (leave.getFrom_date() != null && leave.getFrom_date().matches("\\d{4}-\\d{2}-\\d{2}")) {
	                    leave.setFrom_date(LocalDate.parse(leave.getFrom_date(), inputFormatter).format(outputFormatter));
	                }
	                if (leave.getTo_date() != null && leave.getTo_date().matches("\\d{4}-\\d{2}-\\d{2}")) {
	                    leave.setTo_date(LocalDate.parse(leave.getTo_date(), inputFormatter).format(outputFormatter));
	                }
	                if (leave.getRequested_date() != null && leave.getRequested_date().matches("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}")) {
	                    leave.setRequested_date(LocalDateTime.parse(leave.getRequested_date(), inputDateTimeFormatter).format(outputDateTimeFormatter));
	                }
	            } catch (Exception e) {
	                System.err.println("Error formatting date for leave request: " + leave.getFrom_date() + " - " + e.getMessage());
	            }
	        }
	        

	        // ✅ Print all leave applications after formatting
	        leaves.forEach(leave -> System.out.println("Leave Application: " + leave));
	        return leaves;
	    }
	    ...
	    public List<LeaveApplicationWithProfile> getProccesedleaveRequest(int adminDept, String empid) {
	        List<LeaveApplicationWithProfile> leaves;

	        if (adminDept == 0) {
	            leaves = leaveApplicationWithProfileRepo.getProccessedleaveRequestFoeSA(empid);
	        } else {
	            leaves = leaveApplicationWithProfileRepo.getProccessedleaveRequest(empid);
	        }
	        ...
	        leaves.sort(Comparator.comparing(
	        	    leave -> {
	        	        try {
	        	            return leave.getApproved_date_time() != null && !leave.getApproved_date_time().isEmpty()
	        	                ? LocalDateTime.parse(leave.getApproved_date_time(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
	        	                : LocalDateTime.MIN; // Use MIN to handle null values properly
	        	        } catch (Exception e) {
	        	            System.err.println("Error parsing approved_date_time: " + e.getMessage());
	        ...
	        for (LeaveApplicationWithProfile leave : leaves) {
	            try {
	                if (leave.getFrom_date() != null && !leave.getFrom_date().isEmpty()) {
	                    leave.setFrom_date(LocalDate.parse(leave.getFrom_date(), DateTimeFormatter.ofPattern("yyyy-MM-dd")).format(dateFormatter));
	                }

	                if (leave.getTo_date() != null && !leave.getTo_date().isEmpty()) {
	                    leave.setTo_date(LocalDate.parse(leave.getTo_date(), DateTimeFormatter.ofPattern("yyyy-MM-dd")).format(dateFormatter));
	                }

	                if (leave.getRequested_date() != null && !leave.getRequested_date().isEmpty()) {
	                    leave.setRequested_date(LocalDateTime.parse(leave.getRequested_date(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")).format(dateTimeFormatter));
	                }
	                
	                if (leave.getApproved_date_time() != null && !leave.getApproved_date_time().isEmpty()) {
	                    leave.setApproved_date_time(LocalDateTime.parse(leave.getApproved_date_time(), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")).format(dateTimeFormatter));
	                }

	            } catch (Exception e) {
	                System.err.println("Error formatting date for processed leave request: " + e.getMessage());
	            }
	        }

	        // Sort based on 'way' value in descending order
	        //leaves.sort(Comparator.comparingInt(LeaveApplication::getWay).reversed());
	     // ✅ Print all processed leave applications after formatting
	        leaves.forEach(leave -> System.out.println("Processed Leave Application: " + leave));

	        return leaves;
	    }

	
	    public List<LeaveApplicationWithProfile> getRejectedleaveRequest(int adminDept, String empid) {
	        List<LeaveApplicationWithProfile> leaves;

	        if (adminDept == 0) {
	            leaves = leaveApplicationWithProfileRepo.getRejectedleaveRequestForSA(empid);
	        } else {
	            leaves = leaveApplicationWithProfileRepo.getRejectedleaveRequest(empid);
	        }
	        ...
	        for (LeaveApplicationWithProfile leave : leaves) {
	            try {
	                ...
	                System.err.println("Error formatting date for rejected leave request: " + e.getMessage());
	            }
	        }
	...
	public List<LeaveApplicationWithProfile> findByEmpidNot(String loggedempId) {
		
		List<LeaveApplicationWithProfile> leaves = leaveApplicationWithProfileRepo.findByEmpidNot(loggedempId);    
	...
	public String updateLeaveRequest(LeaveApplication updatedRequest) {
	    ...
	    if (existingLeaveRequestOpt.isPresent()) {
	        ...
	        existingLeaveRequest.setApprovedstatus("Pending");
