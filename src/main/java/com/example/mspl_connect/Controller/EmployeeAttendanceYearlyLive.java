...
public class EmployeeAttendanceYearlyLive {
	 ...
	 public Map<String, Object> getYearlyWorkingHours(@RequestParam("year") int year,@RequestParam("email") String email ,HttpSession session) {
	  ...
	  // Define start and end of the financial year
	     LocalDate startOfYear = LocalDate.of(year, 3, 26);
	     LocalDate endOfYear = LocalDate.of(year + 1, 3, 25);
	     ...
	     double totalWorkingDaysExcludingHolidays = attendanceServiceDto.calculateTotalWorkingDaysExcludingHolidaysAndProbation(startOfYear,empId);
