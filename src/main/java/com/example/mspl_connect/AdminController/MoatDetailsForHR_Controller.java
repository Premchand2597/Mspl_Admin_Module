...
public class MoatDetailsForHR_Controller {
   ...
   /* @PostMapping("/insertLink")
    ...
    public ResponseEntity<String> insertLinkToTable(
        ...
        @RequestParam("encrypted_data") String encryptedData,
        HttpSession session) throws Exception {
    	
    	//System.out.println("holllllllllllllllllllllllllllllllaaaaaaaaaaaaaaaaa");
        ...
        for (String email : emailList) {
        	...
        	if(dept_name != null && !dept_name.isEmpty()) {
        		//System.out.println("Holaaaaaa == "+dept_name);
        		linkGenerator_Service.sendTODB(url, email.trim(), testDuration, supervisorName, testType, encryptedData, dept_name);
        	}else {
        		
        		//System.out.println("Hola == "+dept_name);
        		
        		String email1 = (String) session.getAttribute("email");
        		String empId = employeeRepository.findEmpidByEmail(email1);
        		...
        		linkGenerator_Service.sendTODB(url, email.trim(), testDuration, supervisorName, testType, encryptedData, adminDeptName);
        	}
            
//            try {
//    			encryptedEmail = encrypt(email.trim()); // Encrypting password
//    		} catch (Exception e) {
//    			e.printStackTrace();
//    		}
            
            //sendEmail_Service.sendEmailWithLink(email.trim(), "https://192.168.1.66:9377/moatUserRegister/"+email.trim());
            sendEmail_Service.sendEmailWithLink(email.trim(), "https://www.msplrfdcu.com:9377/moatUserRegister/"+email.trim(), session);
        }
	...
	public String allCandidatePage(HttpSession session, Model model) {
	...
	public String selectedCandidatePage(HttpSession session, Model model) {
		// Safely check if the session attribute is null
	    if (session.getAttribute("empDetailsByEmpId") == null) {
	        return "redirect:/";
	    }
	...
	public String rejectedCandidatePage(HttpSession session, Model model) {
		// Safely check if the session attribute is null
	    if (session.getAttribute("empDetailsByEmpId") == null) {
	        return "redirect:/";
	    }
	...
	public String selectedCandidatePageById(@RequestParam("candidate_id") String candidate_id, Model model, HttpSession session) {
	 ...
	 // Retrieve user details from session
		...
		return "SelectedCandidateProfilePage";
	}
	...
	public String rejectedCandidatePageById(@RequestParam("candidate_id") String candidate_id, Model model, HttpSession session) {
	 ...
	 // Retrieve user details from session
		...
		moatCandidate_Service.update_candidate_flagData(candidate_id);
		model.addAttribute("candidate_id", candidate_id);
		return "RejectedCandidateProfilePage";
	}
	...
	public String totalCandidatePageById(@RequestParam("candidate_id") String candidate_id, @RequestParam("candidateFlagValue") String candidateFlagValue, Model model, HttpSession session) {
	 ...
	 // Retrieve user details from session
	    ...
	    moatCandidate_Service.update_candidate_flagData(candidate_id);
		model.addAttribute("candidate_id", candidate_id);
		return "TotalCandidateProfilePage";
	}
	...
	@GetMapping("/interview")
	 ...
	 public String moatDashboardPage(HttpSession session, Model model) {
		// Safely check if the session attribute is null
		    if (session.getAttribute("empDetailsByEmpId") == null) {
		        return "redirect:/";
		    }
	...
	public ResponseEntity<List<MoatCandidate_Entity>> fetchStudentAllDataBasedOnAction(@RequestParam(value = "action", required = false) String action,HttpSession session) {
	    ...
	    System.out.println(email+" "+empId+" "+adminDept+" "+adminDeptName);
   ...
   @GetMapping("fetchStudentResultById")
	...
	public ResponseEntity<String> toggleApproveStatus(@RequestParam String action_satus, @RequestParam String id, @RequestParam String candidate_action_reason) {
	    try {
	    	...
	    	if(action_satus.equals("approv")) {
	    		approveStudentStatus_Service.setUploadFlag("1", id);
	    	}else if(action_satus.equals("reject") || action_satus.equals("onhold")) {
	    		approveStudentStatus_Service.setUploadFlag("0", id);
	    	}
	    	fetchStudentResult_Service.updateFinalSelectionStatusFlagView(id, "1");
	...
	public String insertMoadeOfContactDetails(@ModelAttribute("modeOfContactNewCandidates_Entity") ModeOfContactNewCandidates_Entity modeOfContactNewCandidates_Entity,HttpSession session) {
		...
		if(modeOfContactNewCandidates_Entity.getDept() != null) {
			modeOfContactNewCandidates_Service.addMoatContactDataForSA(modeOfContactNewCandidates_Entity);
	...
	@GetMapping("/fetchIndividualStatusCountsForInterview")
	   ...
	   getIndividualStatusCountsForInterview(HttpSession session) {
		
		String loggedAdinMail = (String) session.getAttribute("email");
		String empId = employeeRepository.findEmpidByEmail(loggedAdinMail);
		int adminDept = employeeRepository.findDeptIdByEmpId(empId);
		String adminDeptName = departmentRepo.findDeptNameByDeptId(adminDept);
		
	      return cardCountBasedOnInterviewData_Repo.fetchInterviewAllTypeOfStatusCountForCards(adminDeptName);
	   } 

	@GetMapping("/fetchIndividualStatusCountsForModeOfContact")
	   @ResponseBody
	   public CardCountBasedOnModeOfContactData_Entity getIndividualStatusCountsForModeOfContact(HttpSession session) {
		...
		System.out.println(";;;;;;;;;;;;;;;;"+adminDept);
	      return cardCountBasedOnModeOfContactData_Repo.fetchModeOfContactAllTypeOfStatusCountForCards(adminDeptName);
	   }
	
	
	@PostMapping("/updateSelectionStatusInEachRounds")
    public ResponseEntity<String> updateSelectionStatusInEachRoundsDetails(@RequestParam String clickedRound, @RequestParam String student_id, 
                                                                    @RequestParam String status) {
        try {
            boolean isStatusChanged = fetchStudentResult_Service.updateSelectionStatusforCandidateInEachRounds(clickedRound, student_id, status);
            if (isStatusChanged) {
                return ResponseEntity.ok("success");
            } else {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error Updating selection status result in each rounds. Please try again.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error Updating selection status result in each rounds. Please try again.");
    ...
    @PostMapping("/updateInvitationForNextRoundData")
    public ResponseEntity<String> updateInvitationForNextRoundDetails(@RequestParam String selectedRound, @RequestParam String student_id, 
                                                                    @RequestParam String date, @RequestParam String time) {
        try {
            boolean isStatusChanged = fetchStudentResult_Service.updateInvitationForNextRoundDetails(selectedRound, student_id, date, time);
            if (isStatusChanged) {
                return ResponseEntity.ok("success");
            } else {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error Updating Interview date and time for next round. Please try again.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error Updating Interview date and time for next round. Please try again.");
