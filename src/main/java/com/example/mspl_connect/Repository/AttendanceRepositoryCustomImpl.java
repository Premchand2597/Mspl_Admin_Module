...
public class AttendanceRepositoryCustomImpl implements AttendanceRepositoryCustom {
    ...
    public List<AttendanceDTO> findAttendanceByDateAndEmployee(String date, String employeeId) {
    	...
    	String sql = """
    			...
    			SELECT eid, pd AS date, MIN(CASE WHEN pt = min_pt THEN pt ELSE NULL END) AS in_time, CASE WHEN pd::date = CURRENT_DATE THEN '-' ELSE MAX(CASE WHEN pt = max_pt THEN pt ELSE NULL END)
    			END AS out_time FROM ( SELECT eid, pd, pt, (SELECT MIN(pt) FROM attendance WHERE eid = a.eid AND pd = a.pd) AS min_pt, (SELECT MAX(pt) FROM attendance WHERE eid = a.eid AND pd = a.pd) AS max_pt
    			FROM attendance a WHERE eid = ? AND pd = ? ) subquery GROUP BY eid, pd ORDER BY pd;
    			    			
		    			"""; // This will return only the records for the given date
 ...
 // Calculate undertime based on total worked hours and standard hours
    ...
    public List<AttendanceDTO> findAttendanceByyearAndEmployee(int year, String employeeId) {
    	...
    	String sql = """
    			...
    			SELECT eid, pd AS date, MIN(pt) AS in_time, CASE WHEN pd::date = CURRENT_DATE THEN '-' ELSE MAX(pt) END AS out_time FROM attendance WHERE eid = ? AND 
    			extract(YEAR from pd::date) = ? GROUP BY eid, pd ORDER BY pd;
   ...
   /* private static class AttendanceRowMapper implements RowMapper<AttendanceDTO> {
    ...
    public List<AttendanceDTO> findAttendanceByEmployee(String employeeId) {
    	...
    	String sql = """
    			...
    			SELECT eid, pd AS date, MIN(CASE WHEN pt = min_pt THEN pt ELSE NULL END) AS in_time, CASE WHEN pd::date = CURRENT_DATE THEN '-' ELSE 
    			MAX(CASE WHEN pt = max_pt THEN pt ELSE NULL END) END AS out_time FROM ( SELECT eid, pd, pt, MIN(pt) OVER (PARTITION BY eid, pd) AS min_pt,
    			MAX(pt) OVER (PARTITION BY eid, pd) AS max_pt FROM attendance WHERE eid = ? ) subquery GROUP BY eid, pd ORDER BY pd;
	...
	@Override
	public List<AttendanceDTO> findAttendanceByDateRange(LocalDate startDate, LocalDate endDate, String employeeId) {
        /*String sql = "WITH attendance_min_max AS ( " +
                "    SELECT eid, pd, MIN(pt) AS min_pt, MAX(pt) AS max_pt " +
                "    FROM attendance " +
                "    WHERE eid = ? AND pd BETWEEN ? AND ? " + // Use date range instead of month filter
                "    GROUP BY eid, pd " +
                ") " +
                "SELECT am.eid, am.pd AS date, " +
                "    MIN(CASE WHEN a.pt = am.min_pt THEN a.pt ELSE NULL END) AS in_time, " +
                "    CASE " +
                "        WHEN am.pd = CURDATE() THEN '-' " +
                "        ELSE MAX(CASE WHEN a.pt = am.max_pt THEN a.pt ELSE NULL END) " +
		...
		String sql = """
				...
				WITH attendance_min_max AS ( SELECT eid, pd, MIN(pt) AS min_pt, MAX(pt) AS max_pt FROM attendance WHERE eid = ? AND pd::DATE BETWEEN ? AND ? GROUP BY eid, pd )
				SELECT am.eid, am.pd AS date, MIN(CASE WHEN a.pt = am.min_pt THEN a.pt ELSE NULL END) AS in_time, CASE WHEN am.pd::DATE = CURRENT_DATE THEN '-' ELSE MAX(CASE WHEN a.pt = am.max_pt THEN a.pt ELSE NULL END)
				END AS out_time, CONCAT(e.f_Name, ' ', e.l_Name) AS employee_name FROM attendance_min_max am JOIN attendance a ON a.eid = am.eid AND a.pd = am.pd JOIN employee_details e ON e.empId = am.eid GROUP BY am.eid, am.pd, e.f_Name, e.l_Name ORDER BY am.pd;	
				        		
	        		""";

        return jdbcTemplate.query(sql, new Object[]{employeeId, startDate, endDate}, new AttendanceRowMapperWithName());
    }
	
	// Attendance Repository Method
    public List<AttendanceDTO> findAttendanceBetweenDates(String empId, LocalDate startDate, LocalDate endDate) {
    	
        /*String sql = """
            WITH attendance_min_max AS (
                SELECT 
                    eid, 
                    pd, 
                    MIN(pt) AS min_pt, 
    	...
    	String sql = """
                ...
                SELECT 
                    ...
                    MAX(pt) AS max_pt 
                FROM attendance 
                WHERE eid = ? AND pd::date BETWEEN ? AND ? 
                GROUP BY eid, pd
            )
            SELECT 
                am.eid, 
                ...
                MIN(CASE WHEN a.pt = am.min_pt THEN a.pt ELSE NULL END) AS in_time, 
                CASE 
                    WHEN am.pd::date = CURRENT_DATE THEN '-' 
                    ELSE MAX(CASE WHEN a.pt = am.max_pt THEN a.pt ELSE NULL END) 
                END AS out_time, 
                CONCAT(e.f_Name, ' ', e.l_Name) AS employee_name 
            FROM attendance_min_max am 
        ...
        List<AttendanceDTO> records = jdbcTemplate.query(sql, new Object[]{empId, startDate, endDate}, new AttendanceRowMapperWithName());

        records.forEach(dto -> {
        	LocalDate today = LocalDate.now();
        	LocalDate entryDate = LocalDate.parse(dto.getDate());
            ...
            if (dto.getInTime() != null && dto.getOutTime() != null) {
                ...
                double totalHoursWorked = parseDurationToHours(duration);
                String standardHoursStr = "09:00:00";

                if (entryDate.equals(today)) {
                    dto.setUndertime("-");  // Ensure undertime is "-" for today
                } else  if (totalHoursWorked < parseDurationToHours(standardHoursStr)) {
                    dto.setUndertime(calculateUndertime(totalHoursWorked, standardHoursStr));
                } else {
                    dto.setUndertime("-");
                }
            } else {
    ...
    @Override
    public List<AttendanceDTO> findAttendanceByEmployeeNotWithYear(String employeeId) {
    	//System.out.println("heyy");
        /*String sql = "SELECT " +
                "eid, " +
                "pd AS date, " +
                ...
                "END AS out_time " +
                "FROM attendance " +
                "WHERE eid = ? AND pd >= '2025-01-01' " +
                "GROUP BY eid, pd " +
                "ORDER BY pd;";*/
    	
    	String sql = """
        		
        		SELECT eid, pd AS date, MIN(pt) AS in_time, CASE WHEN pd::DATE = CURRENT_DATE THEN '-' ELSE MAX(pt) END AS out_time FROM attendance 
        		WHERE eid = ? AND pd >= '2025-01-01' GROUP BY eid, pd ORDER BY pd;
    ...
    @Override
    public List<AttendanceReportGeneratorFinal_DTO> fetchEmployeeAttendanceForReportGenerator(String startDate, String endDate) {
        String sql = "SELECT concat(ed.f_name, ' ', ed.l_name) as emp_name, a.eid, a.pd, " +
                     "min(a.pt) as punch_in, max(a.pt) as punch_out " +
                     "FROM employee_details ed " +
                     "LEFT JOIN attendance a ON a.eid = ed.empid " +
                     "WHERE ed.role_id <> '0' AND a.pd BETWEEN :startDate AND :endDate " +
                     "GROUP BY emp_name, a.eid, a.pd " +
                     "ORDER BY emp_name";
        ...
        return namedParameterJdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(AttendanceReportGeneratorFinal_DTO.class));
    }

	@Override
	public List<AttendanceReportGeneratorFinal_DTO> fetchEmployeeAttendanceForReportGeneratorBasedOnEmpId(
			String startDate, String endDate, String emp_id) {
		
		String sql = "SELECT concat(ed.f_name, ' ', ed.l_name) as emp_name, a.eid, a.pd, " +
                "min(a.pt) as punch_in, max(a.pt) as punch_out " +
                "FROM employee_details ed " +
                "LEFT JOIN attendance a ON a.eid = ed.empid " +
                "WHERE ed.role_id <> '0' AND a.pd BETWEEN :startDate AND :endDate AND a.eid=:emp_id " +
                "GROUP BY emp_name, a.eid, a.pd " +
                "ORDER BY emp_name";
